...
def create_quizkey_class(app):
    ...
    def __new__(cls, value, quiz_id=None):
        obj = str.__new__(cls, value)
        obj._value_ = value
        obj._quiz_id = quiz_id
        return obj
    ...
    class_dict["quiz_id"] = quiz_id

    def get_id(cls, value):
        """
        Pass a raw string value (e.g., 'definitions').
    ...
    with app.app_context():
        with closing(connect(**app.config['DSN'])) as dbh, closing(dbh.cursor(dictionary=True)) as cursor:
            ...
            for row in rows:
                key = row["quiz_key"].upper().replace(" ", "_")
                # Map the key to a tuple: (string_value, integer_id)
                class_dict[key] = (row["quiz_key"], row["quiz_id"])
